 ...
 */
public class JFXSliderSkinCustom extends BehaviorSkinBase<Slider, SliderBehavior> {
	...
	private double thumbRadius, trackStart, trackLength, thumbTop, thumbLeft, preDragThumbPos, indicatorRotation,
			horizontalRotation, rotationAngle = 45, shifting;
	...
	public JFXSliderSkinCustom(JFXSlider slider) {
	...
	protected void layoutChildren(final double x, final double y, final double w, final double h) {
		...
		if (!initialization) {
			...
			if (isHorizontal) {
				...
				double startY = y + Math.max(h, trackAreaHeight) / 2; // center
																		// slider
																		// in
																		// available
																		// height
				...
				double startX = x + Math.max(w, trackAreaWidth) / 2; // center
																		// slider
																		// in
																		// available
																		// height
	...
	private void initializeVariables() {
		...
		animatedThumb.backgroundProperty()
				.bind(Bindings.createObjectBinding(
						() -> new Background(
								new BackgroundFill(thumb.getStroke(), new CornerRadii(50, 50, 50, 0, true), null)),
						thumb.strokeProperty()));
	...
	private void initializeMouseEvents() {
		...
		thumb.layoutXProperty().addListener((o, oldVal, newVal) -> {
			if (isHorizontal) {
				...
				sliderValue.setText(DataUtils.formatSeconds(value, false));
	...
	private void initializeTimeline() {
		if (isHorizontal) {
			...
			timeline = new Timeline(
					new KeyFrame(Duration.ZERO, new KeyValue(animatedThumb.scaleXProperty(), 0, Interpolator.EASE_BOTH),
					...
					new KeyFrame(Duration.seconds(0.2),
							...
							new KeyValue(animatedThumb.scaleYProperty(), 1, Interpolator.EASE_BOTH), new KeyValue(
									animatedThumb.layoutYProperty(), thumbPosY - shifting, Interpolator.EASE_BOTH)));
		} else {
			...
			timeline = new Timeline(
					...
					new KeyFrame(Duration.seconds(0.2),
							new KeyValue(animatedThumb.scaleXProperty(), 1, Interpolator.EASE_BOTH),
							new KeyValue(animatedThumb.scaleYProperty(), 1, Interpolator.EASE_BOTH), new KeyValue(
									animatedThumb.layoutXProperty(), thumbPosX - shifting, Interpolator.EASE_BOTH)));
		}
	}
	...
	private void positionThumb(final boolean animate) {
		...
		final double endX = (isHorizontal)
				? trackStart + (((trackLength * ((s.getValue() - s.getMin()) / (s.getMax() - s.getMin())))))
				: thumbLeft;
		final double endY = (isHorizontal) ? thumbTop
				: snappedTopInset() + thumbRadius + trackLength
